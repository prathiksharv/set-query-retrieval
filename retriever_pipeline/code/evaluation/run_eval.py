...
from src.common import example_utils
...
def main(unused_argv):
  ...
  for gold_example in gold_examples:
    if not gold_example.docs:
      # raise ValueError(f"{gold_example.query} Example has 0 docs.")
      continue
    
    # breakpoint()

    if gold_example.query not in query_to_pred_example:
      continue

    gold_examples_filt.append(gold_example)
  ...
  print("Avg. Recall")
  eval_utils.print_avg(gold_examples_filt, r_vals)
  eval_utils.print_avg_by_template(gold_examples_filt, r_vals)
  print("Avg. Precision")
  eval_utils.print_avg(gold_examples_filt, p_vals)
  eval_utils.print_avg_by_template(gold_examples_filt, p_vals)
  print("Avg. F1")
  eval_utils.print_avg(gold_examples_filt, f1_vals)
  eval_utils.print_avg_by_template(gold_examples_filt, f1_vals)
